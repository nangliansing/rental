# Update My Saved Search

Partially update a non-deleted, non-closed saved search owned by the authenticated user.
...
```http
PATCH http://localhost:3000/api/v1/saved-searches/6a70f2002c0e518d5b689200
```
...
- Geo/filter field rules are shared with [`create-saved-search.md`](./create-saved-search.md).
...
`status` changes use [`owner-update-saved-search-status.md`](./owner-update-saved-search-status.md). This endpoint never updates `createdBy`.

## Business Rules

1. Caller must own the saved search (`createdBy === req.currentUser._id`)
2. Soft-deleted saved searches cannot be updated (treated as not found)
3. Closed saved searches cannot be updated
...
- [ ] Closed request returns `409 SAVED_SEARCH_CLOSED`
