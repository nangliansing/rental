# Delete My Client Request

Soft-delete a non-deleted client request owned by the authenticated user.
...
```http
DELETE /api/v1/client-requests/:clientRequestId
```

Full local URL:

```http
DELETE http://localhost:3000/api/v1/client-requests/6a70f2002c0e518d5b689200
```
...
The client request must match all of the following in one atomic write filter:
...
These cases all return the same `CLIENT_REQUEST_NOT_FOUND` response:

- the client request does not exist
- the client request belongs to another user
- the client request is already soft-deleted
...
Client request not found, owned by someone else, or already soft-deleted:
...
- [ ] Already-deleted request returns `404 CLIENT_REQUEST_NOT_FOUND`
