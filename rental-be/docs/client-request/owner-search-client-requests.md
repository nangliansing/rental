...
List non-deleted client requests owned by the authenticated user.
...
```http
GET http://localhost:3000/api/v1/client-requests
```
...
```http
GET http://localhost:3000/api/v1/client-requests?page=1&limit=20&status=Waiting
```
...
`ClientRequest` is registered in `scripts/database/index-models.js` so `npm run db:indexes:create` applies them.
