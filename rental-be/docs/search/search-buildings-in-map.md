...
```http
POST http://localhost:3000/api/v1/search/buildings/map
```
...
- active buildings inside `bounds` can be returned with `listings: []`
...
```json
{
  "success": false,
  "code": "VALIDATION_ERROR",
  "message": "bounds must contain exactly: northEast, southWest"
}
```

Invalid bounds coordinate:

```json
{
  "success": false,
  "code": "VALIDATION_ERROR",
  "message": "bounds.northEast.lat must be a valid latitude"
}
```
...
- base anonymous map search returns `200`
- pagination page 1/page 2 works
- page beyond available data returns `200` with `data: []`
- out-of-bounds search returns `200` with `data: []`
- building type filter works
- building facilities filter works
- security filter works
- rent max filter works
- rent range filter works
- pet allowed filter works
- cooking allowed filter works
- foreigner accepted filter works
- TM30 filter works
- bedroom count exact studio behavior works
- bedroom count greater-than-or-equal behavior works
- kitchen type `"Kitchen"` includes `"Separate Kitchen"`
- kitchen type `"No Kitchen"` is exact
- listing facilities filter works
- contract months less-than-or-equal behavior works
- occupancy greater-than-or-equal behavior works
...
- invalid bounds shape returns `422`
- invalid bounds coordinate returns `422`
- invalid page returns `422`
- invalid limit returns `422`
- invalid enum/facility values return `422`
