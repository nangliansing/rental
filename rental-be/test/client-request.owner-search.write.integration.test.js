...
const listPath = "/api/v1/client-requests";
...
const createUser = async ({
  ...
  const user = await User.create({
    name: `Client request list ${role} ${status}`,
...
const seedClientRequest = async ({
  ...
  return ClientRequest.findById(doc._id);
};

const listClientRequests = async ({ token, query = "" }) => {
...
before(async () => {
  ...
  process.env.MONGODB_URI = replSet.getUri(
    "client_request_owner_search_write_test",
  );
...
describe("GET /api/v1/client-requests", () => {
  test("lists only the caller's non-deleted requests", async () => {
    const owner = await createUser();
    const other = await createUser();

    await seedClientRequest({ user: owner.user, name: "Mine" });
    await seedClientRequest({
      user: owner.user,
      name: "Deleted mine",
      isDeleted: true,
    });
    await seedClientRequest({ user: other.user, name: "Theirs" });

    const response = await listClientRequests({ token: owner.token });
  ...
  test("sorts sooner availableBy first and missing availableBy last", async () => {
    ...
    const response = await listClientRequests({ token });
  ...
  test("defaults omitted status to Waiting and can filter Closed", async () => {
    ...
    const defaultsToWaiting = await listClientRequests({ token });
    ...
    const waiting = await listClientRequests({
      token,
    ...
    const closed = await listClientRequests({
      token,
      query: "status=Closed",
    });
  ...
  test("supports pagination", async () => {
    ...
    const page1 = await listClientRequests({
      token,
    ...
    const page2 = await listClientRequests({
      token,
  ...
  for (const [status, expectedCode] of [
    ...
    test(`rejects a ${status.toLowerCase()} user`, async () => {
      const { token, user } = await createUser();
      await seedClientRequest({ user, name: "Mine" });
  ...
  test("returns 422 for an invalid status", async () => {
    ...
    const response = await listClientRequests({
      token,
      query: "status=Open",
    });
