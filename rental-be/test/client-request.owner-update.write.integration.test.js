...
const createUser = async ({
  ...
  const user = await User.create({
    name: `Client request update ${role} ${status}`,
...
before(async () => {
  ...
  process.env.MONGODB_URI = replSet.getUri("client_request_owner_update_write_test");
...
describe("PATCH /api/v1/client-requests/:clientRequestId", () => {
  test("updates editable fields for the owner", async () => {
    ...
    assert.equal(data._id, created.body.data._id);
    assert.equal(data.createdBy, user._id.toString());
    assert.equal(data.name, "Updated request");
    assert.equal(data.description, "New notes");
  ...
  test("rejects unknown immutable fields", async () => {
    const { token } = await createUser();
    const created = await createClientRequest({
      token,
      body: areaBody(),
    ...
    assert.match(response.body.message, /Unknown fields/);
  ...
  test("returns 409 for a closed client request", async () => {
    const { token } = await createUser();
    const created = await createClientRequest({
      token,
      body: areaBody(),
    ...
    assert.equal(response.body.code, "CLIENT_REQUEST_CLOSED");
    assertRequestId(response);
  });
