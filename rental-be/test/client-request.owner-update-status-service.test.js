...
import { validateOwnerUpdateClientRequestStatusBody } from "../modules/client-request/client-request.validation.js";
import { throwClientRequestClosed } from "../modules/client-request/utils/throw-client-request-closed.js";
...
const closedBody = { status: CLIENT_REQUEST_STATUSES.CLOSED };
...
const { ownerUpdateClientRequestStatusService } = await import(
  "../modules/client-request/services/owner-update-client-request-status.service.js"
);
...
const assertClosedError = (error) => {
  ...
  assert.equal(error.message, "Closed client requests cannot be updated");
  return true;
};
...
describe("throwClientRequestClosed", () => {
  test("throws a stable 409 AppError", () => {
    assert.throws(() => throwClientRequestClosed(), assertClosedError);
  });
});
...
describe("validateOwnerUpdateClientRequestStatusBody", () => {
  test("accepts Closed with trimming", () => {
    assert.deepEqual(
      validateOwnerUpdateClientRequestStatusBody({ status: "  Closed  " }),
  ...
  test("rejects null body", () => {
    assert.throws(
      () => validateOwnerUpdateClientRequestStatusBody(null),
  ...
  test("rejects non-object body", () => {
    assert.throws(
      () => validateOwnerUpdateClientRequestStatusBody("Closed"),
      (error) => assertValidationError(error, "body must be an object"),
    );
  });
  ...
  test("rejects missing status", () => {
    assert.throws(
      () => validateOwnerUpdateClientRequestStatusBody({}),
  ...
  test("rejects null status", () => {
    assert.throws(
      () => validateOwnerUpdateClientRequestStatusBody({ status: null }),
      (error) => assertValidationError(error, "status is required"),
    );
  });
  ...
  for (const status of ["", "waiting", "WAITING", "closed", "Open", 1, {}, []]) {
    test(`rejects invalid status: ${JSON.stringify(status)}`, () => {
      assert.throws(
        () => validateOwnerUpdateClientRequestStatusBody({ status }),
...
describe("ownerUpdateClientRequestStatusService", () => {
  describe("successful close", () => {
    test("closes an owned Waiting request atomically", async () => {
      ...
      const result = await ownerUpdateClientRequestStatusService({
    ...
    test("accepts ObjectId inputs", async () => {
      ...
      await ownerUpdateClientRequestStatusService({
        clientRequestId,
        actorId,
        body: closedBody,
      });
    ...
    for (const [label, value] of [
      ...
      test(`rejects invalid clientRequestId: ${label}`, async () => {
        await assert.rejects(
          () =>
            ownerUpdateClientRequestStatusService({
  ...
  describe("write contract", () => {
    ...
    test("write filter always requires Waiting and isDeleted false", async () => {
      ...
      assert.equal(filter.status, CLIENT_REQUEST_STATUSES.WAITING);
