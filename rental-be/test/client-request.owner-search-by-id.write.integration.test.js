...
const createUser = async ({
  ...
  const user = await User.create({
    name: `Client request by id ${role} ${status}`,
...
const getClientRequestById = async ({ token, clientRequestId }) => {
...
before(async () => {
  ...
  process.env.MONGODB_URI = replSet.getUri(
    "client_request_owner_search_by_id_write_test",
  );
...
describe("GET /api/v1/client-requests/:clientRequestId", () => {
  test("returns an owned Waiting client request", async () => {
    ...
    const response = await getClientRequestById({
      token,
  ...
  test("returns an owned Closed client request", async () => {
  ...
  test("returns 404 for a missing client request id", async () => {
    const { token } = await createUser();
