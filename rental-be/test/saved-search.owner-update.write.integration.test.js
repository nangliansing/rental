...
const createUser = async ({
  ...
  const user = await User.create({
    name: `Saved search update ${role} ${status}`,
...
before(async () => {
  ...
  process.env.MONGODB_URI = replSet.getUri("saved_search_owner_update_write_test");
...
describe("PATCH /api/v1/saved-searches/:savedSearchId", () => {
  ...
  test("returns 409 for a closed saved search", async () => {
    const { token } = await createUser();
    const created = await createSavedSearch({
      token,
      body: areaBody(),
    ...
    assert.equal(response.body.code, "SAVED_SEARCH_CLOSED");
    assertRequestId(response);
  });
