...
const createUser = async ({
  ...
  const user = await User.create({
    name: `Client request ${role} ${status}`,
...
const createClientRequest = async ({ token, body, headers = {} }) => {
...
before(async () => {
  ...
  process.env.MONGODB_URI = replSet.getUri("client_request_create_write_test");
...
describe("POST /api/v1/client-requests", () => {
  test("creates an area client request for an authenticated active user", async () => {
    ...
    assert.equal(data.status, CLIENT_REQUEST_STATUSES.WAITING);
    ...
    const saved = await ClientRequest.findById(data._id).lean();
    assert.ok(saved);
  ...
  test("creates nearby and line client requests", async () => {
    const { token, user } = await createUser();

    const nearbyResponse = await createClientRequest({
      token,
      body: {
        name: "Pin search",
        geoSearch: {
    ...
    assert.equal(nearbyResponse.body.data.status, CLIENT_REQUEST_STATUSES.WAITING);
    ...
    const lineResponse = await createClientRequest({
      token,
      body: {
        name: "Line search",
        geoSearch: {
    ...
    const savedCount = await ClientRequest.countDocuments({
      createdBy: user._id,
    });
  ...
  test("ignores client-provided status and always creates Waiting", async () => {
    ...
    assert.equal(response.status, 201);
    assert.equal(response.body.data.status, CLIENT_REQUEST_STATUSES.WAITING);

    const saved = await ClientRequest.findById(response.body.data._id).lean();
    assert.equal(saved.status, CLIENT_REQUEST_STATUSES.WAITING);
  });

  test("allows multiple client requests for the same user", async () => {
    const { token, user } = await createUser();

    const first = await createClientRequest({
      token,
    ...
    assert.equal(await ClientRequest.countDocuments({ createdBy: user._id }), 2);
  });
  ...
  test("requires an access token", async () => {
    ...
    assert.equal(await ClientRequest.countDocuments(), 0);
  });
  ...
  test("returns 422 for invalid geoSearch payloads", async () => {
    ...
    const missingBounds = await createClientRequest({
      token,
      body: {
    ...
    const missingRadius = await createClientRequest({
      token,
      body: {
