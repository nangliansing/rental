...
const createUser = async ({
  ...
  const user = await User.create({
    name: `Saved search status ${role} ${status}`,
...
const updateSavedSearchStatus = async ({
  token,
  savedSearchId,
  body,
}) => {
  return request(`${createPath}/${savedSearchId}/status`, {
    method: "PATCH",
    headers: bearerHeaders(token),
    body: JSON.stringify(body),
  });
...
before(async () => {
  ...
  process.env.MONGODB_URI = replSet.getUri(
    "saved_search_owner_update_status_write_test",
  );
...
describe("PATCH /api/v1/saved-searches/:savedSearchId/status", () => {
  test("closes an owned Waiting saved search", async () => {
    ...
    assert.equal(created.body.data.status, SAVED_SEARCH_STATUSES.WAITING);

    const response = await updateSavedSearchStatus({
      token,
      savedSearchId: created.body.data._id,
      body: { status: SAVED_SEARCH_STATUSES.CLOSED },
    });
  ...
  test("returns 409 when the saved search is already Closed", async () => {
    ...
    const first = await updateSavedSearchStatus({
      token,
      savedSearchId: created.body.data._id,
      body: { status: SAVED_SEARCH_STATUSES.CLOSED },
    });
    ...
    const second = await updateSavedSearchStatus({
      token,
      savedSearchId: created.body.data._id,
      body: { status: SAVED_SEARCH_STATUSES.CLOSED },
    });
    ...
    assert.equal(second.body.code, "SAVED_SEARCH_CLOSED");
  ...
  test("returns 404 for a soft-deleted saved search", async () => {
    const { token } = await createUser();
    const created = await createSavedSearch({
      token,
      body: areaBody(),
  ...
  test("closed requests cannot be content-updated afterward", async () => {
    ...
    const closed = await updateSavedSearchStatus({
      token,
      savedSearchId: created.body.data._id,
      body: { status: SAVED_SEARCH_STATUSES.CLOSED },
    });
    ...
    assert.equal(updated.body.code, "SAVED_SEARCH_CLOSED");
  });
  ...
  test("rejects Waiting and unknown body fields", async () => {
    ...
    const waiting = await updateSavedSearchStatus({
      token,
      savedSearchId: created.body.data._id,
    ...
    const unknown = await updateSavedSearchStatus({
      token,
      savedSearchId: created.body.data._id,
      body: {
  ...
  test("requires an access token", async () => {
    const response = await request(
      `${createPath}/${new mongoose.Types.ObjectId().toString()}/status`,
      {
        ...
        body: JSON.stringify({ status: SAVED_SEARCH_STATUSES.CLOSED }),
      },
    );
