...
const createUser = async ({
  ...
  const user = await User.create({
    name: `Saved search by id ${role} ${status}`,
...
const getSavedSearchById = async ({ token, savedSearchId }) => {
...
before(async () => {
  ...
  process.env.MONGODB_URI = replSet.getUri(
    "saved_search_owner_search_by_id_write_test",
  );
...
describe("GET /api/v1/saved-searches/:savedSearchId", () => {
  test("returns an owned Waiting saved search", async () => {
  ...
  test("returns 404 for a missing saved search id", async () => {
    const { token } = await createUser();

    const response = await getSavedSearchById({
      token,
      savedSearchId: new mongoose.Types.ObjectId().toString(),
    });
