...
const confirmButtonToneClasses: Record<ConfirmationDialogTone, string> = {
  neutral: "bg-slate-950 text-white hover:bg-slate-800 disabled:bg-slate-400",
  ...
  return (
    ...
    >
          ...
          <div className="mt-5 grid grid-cols-2 gap-2">
            <button
              type="button"
              className="flex h-11 items-center justify-center rounded-full border border-slate-200 bg-white px-4 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60"
              disabled={isSubmitting}
              onClick={onClose}
            >
              {cancelLabel}
            ...
            <button
              type="button"
              className={cn(
                "flex h-11 items-center justify-center gap-2 rounded-full px-4 text-sm font-semibold transition disabled:cursor-not-allowed",
