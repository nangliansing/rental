...
function renderLayout(
  overrides: Partial<ComponentProps<typeof SavedSearchWizardLayout>> = {},
) {
...
describe("SavedSearchWizardLayout", () => {
  describe("content & chrome", () => {
    it("renders children below the form header", () => {
      ...
      expect(
        screen.getByRole("heading", { name: "Create saved search" }),
      ).toBeInTheDocument()
      expect(
    ...
    it("exports dialog content classes for full-screen mobile / fixed-height desktop", () => {
      expect(SAVED_SEARCH_WIZARD_DIALOG_CONTENT_CLASSNAME).toContain("h-dvh")
      expect(SAVED_SEARCH_WIZARD_DIALOG_CONTENT_CLASSNAME).toContain(
        "rounded-none",
      )
      expect(SAVED_SEARCH_WIZARD_DIALOG_CONTENT_CLASSNAME).toContain(
        "sm:rounded-2xl",
      ...
      expect(SAVED_SEARCH_WIZARD_DIALOG_CONTENT_CLASSNAME).not.toContain(
        "sm:h-auto",
      )
    })
  })
  ...
  describe("header semantics", () => {
    it("defaults to page heading elements for create / edit routes", () => {
      ...
      expect(
        screen.getByRole("heading", { level: 2, name: "Create saved search" }),
      ).toBeInTheDocument()
    })
  ...
  describe("close control", () => {
    ...
    it("uses a custom close aria-label when provided", () => {
      const { onClose } = renderLayout({
        closeAriaLabel: "Close create saved search",
      })

      fireEvent.click(
        screen.getByRole("button", { name: "Close create saved search" }),
      )
