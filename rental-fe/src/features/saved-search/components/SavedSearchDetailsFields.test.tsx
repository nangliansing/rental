...
function renderFields(
  overrides: Partial<ComponentProps<typeof SavedSearchDetailsFields>> = {},
) {
  ...
  const result = render(
    <SavedSearchDetailsFields
      name=""
      description=""
      nameId="saved-search-name"
      descriptionId="saved-search-notes"
...
describe("SavedSearchDetailsFields", () => {
  ...
  describe("maxLength contracts", () => {
    it("caps name and notes at the API max lengths", () => {
      ...
      expect(screen.getByLabelText(/^Name/)).toHaveAttribute(
        "maxLength",
        String(SAVED_SEARCH_NAME_MAX_LENGTH),
      )
      expect(screen.getByLabelText(/^Notes$/)).toHaveAttribute(
        "maxLength",
        String(SAVED_SEARCH_DESCRIPTION_MAX_LENGTH),
      )
    })
  })
  ...
  describe("errors", () => {
    ...
    it("shows a notes error and marks the notes field invalid", () => {
      ...
      expect(screen.getByRole("alert")).toHaveTextContent(
        `Notes must be at most ${SAVED_SEARCH_DESCRIPTION_MAX_LENGTH} characters.`,
      )
      expect(screen.getByLabelText(/^Notes$/)).toHaveAttribute(
        "aria-invalid",
        "true",
