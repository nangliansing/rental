...
import { updateOwnerSavedSearchStatus } from "./updateOwnerSavedSearchStatus"

const closedSavedSearch = {
...
describe("updateOwnerSavedSearchStatus", () => {
  it("patches Closed status and returns the unwrapped SavedSearch", async () => {
    server.use(
      http.patch(
        "/api/v1/saved-searches/:savedSearchId/status",
        async ({ params, request }) => {
          expect(params.savedSearchId).toBe(closedSavedSearch._id)
          await expect(request.json()).resolves.toEqual({ status: "Closed" })
          return HttpResponse.json({
            success: true,
            data: closedSavedSearch,
          })
        },
      ),
    )
    ...
    await expect(
      updateOwnerSavedSearchStatus({
        savedSearchId: `  ${closedSavedSearch._id}  `,
        status: "Closed",
      }),
    ).resolves.toMatchObject({
      _id: closedSavedSearch._id,
      status: "Closed",
      name: "Sukhumvit 2BR",
    })
  })
  ...
  it.each([
    ...
    ["a b", "a b"],
    ["id?x=1", "id?x=1"],
  ])("encodes saved search id %j", async (input, expectedParam) => {
    server.use(
      http.patch(
        "/api/v1/saved-searches/:savedSearchId/status",
        ({ params }) => {
          expect(params.savedSearchId).toBe(expectedParam)
          return HttpResponse.json({
            success: true,
            data: { ...closedSavedSearch, _id: expectedParam },
          })
        },
      ),
    )

    await updateOwnerSavedSearchStatus({
      savedSearchId: input,
      status: "Closed",
    })
  })
  ...
  it.each(["", "   ", "\n\t"])(
    ...
    async savedSearchId => {
      const request = vi.fn()
      server.use(
        http.patch("/api/v1/saved-searches/:savedSearchId/status", request),
      )
  ...
  it.each([
    ...
    [{ success: false, data: closedSavedSearch }],
    [{ success: true }],
    [null],
  ])("rejects malformed success responses (%j)", async body => {
    server.use(
      http.patch("/api/v1/saved-searches/:savedSearchId/status", () =>
