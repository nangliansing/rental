...
function setup(options?: {
  ...
  if (seedClosed) {
    queryClient.setQueryData(
      closedKey,
      listData(clientRequest("old-closed", { status: "Closed" })),
    )
  }
  if (seedDetail) {
  ...
  return {
    ...renderHook(() => useUpdateOwnerClientRequest(), { wrapper: Wrapper }),
    closedKey,
    detailKey,
    queryClient,
    source,
...
describe("useUpdateOwnerClientRequest", () => {
  beforeEach(() => {
  ...
  it("cancels in-flight queries and optimistically patches all Waiting lists + detail", async () => {
    ...
    mocks.updateOwnerClientRequest.mockReturnValue(
  ...
  it("patches an item on a later infinite page without changing totals", async () => {
    mocks.updateOwnerClientRequest.mockReturnValue(new Promise(() => undefined))
  ...
  it("clears description optimistically with blank/null values", async () => {
    ...
    queryClient.setQueryData(
      detailKey,
      clientRequest("request-1", { description: "Old notes" }),
    )
    queryClient.setQueryData(
      waitingKey,
      listData(clientRequest("request-1", { description: "Old notes" })),
    )
  ...
  it("is a no-op on existing caches when the request is not loaded, then reconciles detail", async () => {
    mocks.updateOwnerClientRequest.mockResolvedValue(
      clientRequest("missing", { name: "Server missing" }),
    )
  ...
  it("restores all Waiting lists and detail after failure", async () => {
    mocks.updateOwnerClientRequest.mockRejectedValue(new Error("Network error"))
    const {
      result,
      queryClient,
      waitingKey,
  ...
  it.each([
    ...
    mocks.updateOwnerClientRequest.mockRejectedValue(
  ...
  it("rolls back a Closed detail optimistic patch after CLIENT_REQUEST_CLOSED", async () => {
    mocks.updateOwnerClientRequest.mockRejectedValue(
      new ApiError(
        "This client request is already closed.",
        409,
    ...
    const closed = clientRequest("request-1", {
      status: "Closed",
      name: "Already closed",
    })
  ...
  it("serializes repeated updates through the write scope", async () => {
    ...
    mocks.updateOwnerClientRequest.mockImplementation(
      ({ clientRequestId }: { clientRequestId: string }) =>
        clientRequestId.trim() === "request-1"
          ...
          : Promise.resolve(
              clientRequest(clientRequestId.trim(), { name: "Second done" }),
            ),
    )
    const { result, queryClient } = setup()
    queryClient.setQueryData(
    ...
    await act(async () =>
      resolveFirst(clientRequest("request-1", { name: "First done" })),
    )
    await waitFor(() =>
      expect(mocks.updateOwnerClientRequest).toHaveBeenCalledTimes(2),
    )
  })
