...
describe("buildCreateOwnerClientRequestPayload", () => {
  ...
  it("accepts boundary-length name, description, and placeName", () => {
    const payload = buildCreateOwnerClientRequestPayload({
  ...
  it.each([
    ...
    [
      "name too long",
      { ...areaInput, name: "x".repeat(CLIENT_REQUEST_NAME_MAX_LENGTH + 1) },
    ...
    [
      ...
      {
        ...areaInput,
        description: "x".repeat(CLIENT_REQUEST_DESCRIPTION_MAX_LENGTH + 1),
      },
    ...
    [
      "placeName too long",
      {
        ...
        geoSearch: {
          ...areaInput.geoSearch,
          placeName: "p".repeat(CLIENT_REQUEST_PLACE_NAME_MAX_LENGTH + 1),
        },
      },
    ...
    expectValidationError(
      () => buildCreateOwnerClientRequestPayload(input),
      message,
    )
  })
})

describe("buildCreateOwnerClientRequestGeoSearch", () => {
...
describe("createOwnerClientRequest", () => {
  it("posts a normalized area payload and returns the unwrapped ClientRequest", async () => {
    server.use(
      http.post("/api/v1/client-requests", async ({ request }) => {
    ...
    await expect(createOwnerClientRequest(areaInput)).resolves.toMatchObject({
      _id: createdClientRequest._id,
      name: "Sukhumvit 2BR",
      status: "Waiting",
    })
  })
  ...
  it("posts nearby and line payloads without omitted blanks", async () => {
    ...
    await createOwnerClientRequest({
      name: "Nearby",
      description: null,
      geoSearch: {
        mode: "nearby",
  ...
  it("rejects invalid input before requesting", async () => {
    const request = vi.fn()
    server.use(http.post("/api/v1/client-requests", request))

    await expect(
      createOwnerClientRequest({ ...areaInput, name: " " }),
  ...
  it.each([
    ...
    [{ success: false, data: createdClientRequest }],
    [{ success: true }],
    [null],
  ])("rejects malformed success responses (%j)", async body => {
    server.use(
      http.post("/api/v1/client-requests", () => HttpResponse.json(body)),
    )

    await expect(createOwnerClientRequest(areaInput)).rejects.toMatchObject({
      status: 500,
  ...
  it.each([
    ...
    server.use(
      http.post("/api/v1/client-requests", () =>
        HttpResponse.json(
          {
            success: false,
            code,
...
describe("createOwnerClientRequest source contract", () => {
  it("keeps the mutation hook free of query-cache writes", () => {
    const source = readFileSync(
      join(
        process.cwd(),
        "src/features/client-request/api/useCreateOwnerClientRequest.ts",
      ),
      "utf8",
    )
    const code = source
    ...
    expect(code).toMatch(
      /scope:\s*\{\s*id:\s*CREATE_OWNER_CLIENT_REQUEST_SCOPE_ID/,
    )
  })
})
