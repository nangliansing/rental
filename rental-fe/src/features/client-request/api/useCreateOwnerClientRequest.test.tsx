...
const createOwnerClientRequest = vi.hoisted(() => vi.fn())

vi.mock("./createOwnerClientRequest", () => ({
...
describe("useCreateOwnerClientRequest", () => {
  beforeEach(() => {
    createOwnerClientRequest.mockReset()
  })

  it("exposes pending/success mutation state without touching query cache", async () => {
    let resolveCreate:
      | ((value: typeof createdClientRequest) => void)
      | undefined
    createOwnerClientRequest.mockImplementation(
      () =>
        new Promise(resolve => {
    ...
    const { result } = renderHook(() => useCreateOwnerClientRequest(), {
      wrapper: Wrapper,
    })
    ...
    expect(createOwnerClientRequest).toHaveBeenCalledWith(
      createInput,
      expect.anything(),
    )

    await act(async () => {
      resolveCreate?.(createdClientRequest)
    })

    await waitFor(() => expect(result.current.isSuccess).toBe(true))
    expect(result.current.data).toEqual(createdClientRequest)
  ...
  it("surfaces fetcher errors through mutation state without cache writes", async () => {
    createOwnerClientRequest.mockRejectedValueOnce(
  ...
  it.each([
    ...
    [500, "INVALID_CLIENT_REQUEST_RESPONSE"],
  ] as const)(
  ...
  it("resets mutation state after success and error", async () => {
    createOwnerClientRequest.mockResolvedValueOnce(createdClientRequest)
    ...
    createOwnerClientRequest.mockRejectedValueOnce(
      new ApiError("boom", 500, "INVALID_CLIENT_REQUEST_RESPONSE"),
    )
  ...
  it("serializes concurrent creates through the shared mutation scope", async () => {
    ...
    createOwnerClientRequest.mockImplementation(async (input: { name: string }) => {
      ...
      return { ...createdClientRequest, name: input.name }
    })

    const { Wrapper } = createWrapper()
    const first = renderHook(() => useCreateOwnerClientRequest(), {
      wrapper: Wrapper,
    })
    const second = renderHook(() => useCreateOwnerClientRequest(), {
      wrapper: Wrapper,
    })
    ...
    expect(CREATE_OWNER_CLIENT_REQUEST_SCOPE_ID).toBe(
